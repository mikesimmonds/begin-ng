...
@Component({
  ...
  template: `
    ...
    ></div>
    <div
      *ngIf="options.payload?.error"
      role="alertdialog"
      aria-live="polite"
      [class]="options.messageClass"
      aria-label="show full error"
      (click)="showError($event); showFullError = true"
    >
      View error
    </div>
    <div *ngIf="showFullError" aria-label="Full error">
...
export class ErrorToast implements OnDestroy {
  ...
  showError(ev: Event) {
    ...
    this.toastClasses = this.toastClasses + ' showError';
